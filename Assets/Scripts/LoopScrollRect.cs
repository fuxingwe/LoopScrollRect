...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected virtual bool UpdateItems(Bounds viewBounds, Bounds contentBounds, bool bForward) { return false; }
        //==========LoopScrollRect==========
        ...
        protected virtual void SetContentAnchoredPosition(Vector2 position)
        {
            ...
            if (position != m_Content.anchoredPosition)
            {
                ...
                if (directionSign == 1)
                {
                    bForward = m_Content.anchoredPosition.x > position.x;
                }
                else if (directionSign == -1)
                {
                    bForward = m_Content.anchoredPosition.y > position.y;
                }
                m_Content.anchoredPosition = position;
                UpdateBounds(true, bForward);
            }
        }
        ...
        private void SetNormalizedPosition(float value, int axis)
        {
            ...
            {
                bool bForward = localPosition[axis] > newLocalPosition;
        ...
        private void UpdateBounds(bool updateItems = false , bool bForward = false)
        {
            m_ViewBounds = new Bounds(viewRect.rect.center, viewRect.rect.size);
            m_ContentBounds = GetBounds();
            ...
            if (Application.isPlaying && updateItems && UpdateItems(m_ViewBounds, m_ContentBounds, bForward))
            {
                Canvas.ForceUpdateCanvases();
                m_ContentBounds = GetBounds();
            }
